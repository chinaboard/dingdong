...
```
dd_log_init         hooks ESP_LOG into a 4 KB ring buffer (early so boot logs are captured)
...
- `ble` — NimBLE host. Roles: **Peripheral** (HID adv on instance 0, legacy_pdu for iPhone backward-compat), **Observer** (continuous scan for the presence machine).
  ...
  - **Presence machine**: per-bond slot table with `last_seen_us`, `present`, `ack_event`, `probing`. `dd_ble_presence_reset_events()` (called by `events_wipe`) clears `ack_event` so the next tick re-emits IN for everyone in proximity. iOS goes very sparse on BLE adv when locked + idle (Find My can throttle to under one packet per 30 s), so the tick fires an active connect-probe at `PRESENCE_PROBE_AFTER_MS = 20 s` of staleness, repeating every `PRESENCE_PROBE_COOLDOWN_MS = 20 s` until success or `PRESENCE_TIMEOUT_MS = 60 s`. Each probe stops the scanner via `ble_gap_disc_cancel`, runs `ble_gap_connect` to the peer's identity address (iOS accepts on a known bond without user prompt), refreshes `last_seen` on success, then immediately disconnects and restarts the scanner via `schedule_scan_restart`. Probe is skipped when one is already in flight or during the pairing window. Belt-and-braces third signal: iOS auto-reconnects to known HID peripherals every few minutes when the screen is off — that GAP CONNECT also refreshes `last_seen` for the matching slot. OUT only fires when scan + probe + HID-reconnect all stay silent across the full 60-second window (typically two probe attempts).
